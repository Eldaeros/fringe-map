...
export const HexGrid: FunctionComponent<HexGridProps> = (props: HexGridProps) => {
    ...
    for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
        for (let colIndex = 0; colIndex < columns; colIndex++) {
            ...
            const xGridCoord = (rowIndex % 2 ? colIndex * 2 +2 : colIndex * 2 + 1);
            const yGridCoord = Math.floor(rowIndex / 2) + 1;
...
const HexagonContainer = styled.div`
    ...
    > div {
        // font-weight: ${(props: HexagonContainerProps) => props.hover ? `600` : `normal`}
        color: ${(props: HexagonContainerProps) => props.hover ? `#b5b5b5` : ``}
    }
